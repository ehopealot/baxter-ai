...
You are running in an isolated container with your usual abilities: `code-cli` (offline Python/Node sandbox), the browsers (`playwright-cli` / `invisible-cli`), `WebSearch`/`WebFetch`, your Discord CLI (`discord-cli`), and your email CLI (`node {{GMAIL_CLI_PATH}} ...`). Act freely and directly.
