...
- **openrouter/local (structured-tool) harnesses — NOT reachable (the deployed posture; verified).** `read_file`/`edit_file` and `files-cli` resolve every path under the run's cwd (`~/.mail-agent/memory-workspace/`) and refuse escapes; `code-cli` reaches a remote executor through a Unix signer socket, while the signing key lives only in a signer-only host env file that is not mounted into the app; `playwright-cli`/`invisible-cli` block the `file:` scheme (tested — `Access to "file:" protocol is blocked`). The key files live one level UP (`~/.mail-agent/`), so no read primitive reaches them — the run genuinely cannot read the key. Baxter runs here.
