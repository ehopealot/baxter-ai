...
- **openrouter/local (structured-tool) harnesses — filesystem key files are NOT reachable (the deployed posture; verified).** `read_file`/`edit_file` and `files-cli` resolve every path under the run's cwd (`~/.mail-agent/memory-workspace/`) and refuse escapes; `playwright-cli`/`invisible-cli` block the `file:` scheme (tested — `Access to "file:" protocol is blocked`). The mail/Discord/data key files live one level UP (`~/.mail-agent/`), so no read primitive reaches them. The executor credential is intentionally different: Compose injects its tenant-scoped direct-signing variables into the application container and `code-cli` signs the remote request directly. Treat those variables as reachable within a run's app-container trust boundary. They can submit only that tenant's bounded executor jobs and cannot administer Worker credentials or cross tenant boundaries; this accepted exposure is narrower than the app's other credentials.
