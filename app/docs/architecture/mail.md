...
- Each admitted inbound run receives a temporary mode-0600 proactive-follow-up context bound to its exact indexed thread; only the context path enters the run environment and cleanup runs in `finally`. `followup-cli` can create/list bounded metadata but cannot accept recipient/thread/timezone flags. When such a task fires, code revalidates the same indexed correspondent and calls the normal Resend reply path, preserving the thread with no fallback. See [proactive follow-ups](proactive-followups.md).
