...
- Each admitted inbound run receives its canonical allowlisted email address as the trusted proactive-follow-up delivery target. `followup-cli` accepts no recipient, thread, provider, or timezone flags; it can only write an ordinary one-shot scheduler task using that daemon-supplied mail route. The normal heartbeat path later executes and delivers the task to that canonical address, with ordinary retry, give-up, and cancellation behavior. See [proactive follow-ups](proactive-followups.md).
