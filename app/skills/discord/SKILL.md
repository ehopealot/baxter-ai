...
| `… \| discord-cli dm <userId>` | Send a **direct message** to a user (opens/reuses the DM channel, body on stdin; `--file` works too). May fail if they have DMs from the bot off -- that's fine, treat it as best-effort. |
