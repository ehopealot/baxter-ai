...
import { runAgent, fillTemplate, getHarness } from "../scripts/runtime.mjs";
import { DISCORD_TOOLS, HEARTBEAT_TOOLS, MAIL_TOOLS, MAIL_CLI } from "../scripts/grants.mjs";
import { captureFromEvents, runAssertions } from "./assertions.mjs";

const EVAL_DIR = dirname(fileURLToPath(import.meta.url));
const APP_DIR = dirname(EVAL_DIR); // evals/ -> app/
const MOCK_HANDLER = join(EVAL_DIR, "mock.mjs");
...
// CONVERT the absolute `Bash(node <path>.mjs *)` credential-CLI grants to their
...
// cli; (2) `node <mail.mjs>` has NO friendly grant, so we'd lose mail entirely by
...
// lives in the caller (run.mjs) -- the harness only emits the events.
