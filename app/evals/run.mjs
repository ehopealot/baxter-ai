#!/usr/bin/env node
// `make eval` entrypoint. Loads evals/scenarios/*.mjs, runs the suite on the pinned
...
//   node evals/run.mjs --scenario <substr>    run only matching scenarios
...
const scenarios = [];
for (const f of readdirSync(SCEN_DIR).filter((f) => f.endsWith(".mjs")).sort()) {
