...
- Discover **ecosystem skills** with `skills-cli find <query>` (see the skill-discovery skill): searches the open agent-skills registry (skills.sh) and returns metadata. You can only *find and suggest* — there's no install verb; installing is Erik's call on the host. A trusted-owner match you may recommend (give Erik its `installCommand` verbatim); an untrusted owner → just print the command, no endorsement. **Never** fetch or copy a third-party `SKILL.md` into your workspace — suggest to Erik instead.
