...
test("mail grants mail + schedule-cli, never discord", () => {
  assert.match(MAIL_TOOLS, /Bash\(node \S*mail\.ts \*\)/);
...
test("tui grants the generous operator union (mail + discord + schedule + all core) and all baked skills", () => {
  for (const t of ["Bash(schedule-cli *)", "Bash(discord-cli *)", "Bash(code-cli *)", "Bash(files-cli *)", "Bash(projects-cli *)", "Bash(memory-cli *)", "Bash(calendar-cli *)", "Bash(checklist-cli *)", "Bash(data-cli *)", "Skill", "Read", "Write", "Edit"]) {
