...
//      generic renderer in runtime.ts (parseEvents),
//   3. how we read its terminal usage/rate-limit signal (detectOutcome).
// runAgent (runtime.ts) drives everything through this object's shape, so a
...
//
// DELIBERATELY NOT HERE -- two Claude-Code-isms stay caller-side (poll.ts /
// discord-bot.ts / heartbeat.ts), and a second adapter must account for both:
...
export const claudeHarness = {
  ...
  // limit) signal, so poll.ts can auto-reply instead of the message being
