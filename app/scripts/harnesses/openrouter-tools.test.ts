...
// ToolResult's non-`ok` fields are deliberately loose in runner-common.ts (a
// cross-cluster shared shape read by two different runners' own result kinds);
// narrow locally to what these tests assert on, and read each executor's result
// through it, so assertions get real string/number types instead of `unknown`.
interface R {
  ok: boolean;
  error?: string;
  stdout?: string;
...
const asR = (res: unknown): R => res as R;

test("tokenizeAllowedTools keeps Bash(...) groups intact despite inner spaces", () => {
  const toks = tokenizeAllowedTools("Bash(node /x/mail.ts *) Bash(discord-cli *) WebSearch Read");
  assert.deepEqual(toks, ["Bash(node /x/mail.ts *)", "Bash(discord-cli *)", "WebSearch", "Read"]);
});
...
test("parseAllowedTools builds the CLI allowlist + native set from a real allowedTools string", () => {
  const { cliMap, native } = parseAllowedTools(
    "Bash(node /app/scripts/mail.ts *) Bash(discord-cli *) Bash(code-cli *) WebSearch WebFetch Skill Read Write Edit",
  );
  ...
  assert.deepEqual(cliMap["mail"], { command: "node", prefixArgs: ["/app/scripts/mail.ts"] });
...
test("runCli refuses a cli that isn't in the allowlist", async () => {
  const r = asR(await runCli({ cli: "rm", args: ["-rf", "/"] }, { cliMap: { "discord-cli": { command: "discord-cli", prefixArgs: [] } } }));
...
test("runCli cleanly refuses a prototype-key cli name (constructor/hasOwnProperty)", async () => {
  ...
  const r = asR(await runCli({ cli: "constructor" }, { cliMap }));
  assert.equal(r.ok, false);
  assert.match(r.error!, /not allowed/);
});
...
test("runCli runs an allowed cli with prefixArgs + args and captures stdout/exit", async () => {
  ...
  const r = asR(await runCli({ cli: "echo", args: ["a", "b"] }, { cliMap, maxBytes: 1 << 20, timeoutMs: 5000 }));
...
test("runCli feeds stdin and surfaces a nonzero exit without throwing", async () => {
  ...
  const r = asR(await runCli({ cli: "catn", stdin: "hello" }, { cliMap, maxBytes: 1 << 20, timeoutMs: 5000 }));
...
test("runCli flags truncation when output exceeds the byte cap (even in one chunk)", async () => {
  ...
  const r = asR(await runCli({ cli: "big" }, { cliMap, maxBytes: 100, timeoutMs: 5000 }));
...
test("editFile writes $-patterns in new_string literally (no replacement-special expansion)", () => {
  ...
  assert.equal(asR(editFile({ path: "f.txt", old_string: "PRICE", new_string: "costs $$40 and $& stays" }, ctx)).ok, true);
...
test("writeFile creates missing parent directories (a learned skill's own subdir)", () => {
  ...
  const res = asR(writeFile({ path: "learned-skills/data-cli-espn/SKILL.md", content: "# skill" }, ctx));
  ...
  assert.equal(asR(writeFile({ path: "../evil/x.md", content: "x" }, ctx)).ok, false);
...
test("readFile caps content at ctx.maxBytes and flags truncation (uncapped when maxBytes unset)", () => {
  ...
  const capped = asR(readFile({ path: "big.txt" }, { cwd, maxBytes: 100 }));
  assert.equal(capped.ok, true);
  assert.equal(capped.content!.length, 100);
  assert.equal(capped.truncated, true);
  const full = asR(readFile({ path: "big.txt" }, { cwd })); // no cap -> whole file (back-compat)
...
test("loadSkill caps SKILL.md at ctx.maxBytes", () => {
  ...
  const r = asR(loadSkill({ name: "demo" }, { cwd, maxBytes: 100 }));
  assert.equal(r.ok, true);
...
test("read/write/edit round-trip within cwd, and reject an escape", () => {
  ...
  const ctx = { cwd };
  assert.equal(asR(writeFile({ path: "note.md", content: "one two" }, ctx)).ok, true);
  assert.equal(asR(readFile({ path: "note.md" }, ctx)).content, "one two");
  assert.equal(asR(editFile({ path: "note.md", old_string: "two", new_string: "three" }, ctx)).ok, true);
  assert.equal(readFileSync(join(cwd, "note.md"), "utf8"), "one three");
  // escape refused
  const esc = asR(readFile({ path: "../../etc/passwd" }, ctx));
  ...
  assert.match(asR(editFile({ path: "dup.md", old_string: "x", new_string: "y" }, ctx)).error!, /not unique/);
});
...
test("writes into .claude/ are refused (no self-authored trusted skill), reads stay allowed", () => {
  ...
  const w = asR(writeFile({ path: ".claude/skills/discord/SKILL.md", content: "# poisoned" }, ctx));
  ...
  assert.equal(asR(editFile({ path: ".claude/skills/discord/SKILL.md", old_string: "real", new_string: "poison" }, ctx)).ok, false);
  // reads of .claude/ still work (load_skill / read_file)
  assert.equal(asR(loadSkill({ name: "discord" }, ctx)).content, "# real");
  assert.equal(asR(readFile({ path: ".claude/skills/discord/SKILL.md" }, ctx)).content, "# real");
  // a normal write elsewhere in cwd still works (the guard is .claude/-specific)
  assert.equal(asR(writeFile({ path: "memory.md", content: "ok" }, ctx)).ok, true);
});
...
test("loadSkill reads a staged SKILL.md and can't be traversed out of the skills dir", () => {
  ...
  assert.equal(asR(loadSkill({ name: "discord" }, { cwd })).content, "# discord skill");
