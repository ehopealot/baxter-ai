...
export function systemPreamble(cliMap) {
  ...
  return [
    ...
    "- Web research the instructions call `WebSearch`/`WebFetch` (or \"read a page's content\") is the **web-cli** CLI via run_cli: run_cli({cli:\"web-cli\", args:[\"fetch\", \"<url>\"]}) to read a page. To SEARCH, web-cli's own `search` is disabled -- open Bing in the browser: run_cli({cli:\"playwright-cli\", args:[\"open\", \"https://www.bing.com/search?q=<query>\"]}) then run_cli({cli:\"playwright-cli\", args:[\"snapshot\"]}). Use **playwright-cli** for search (Bing serves it fine), NOT invisible-cli. For a JavaScript-heavy page use playwright-cli; reach for invisible-cli only for a SPECIFIC site that actively bot-walls playwright-cli (a Cloudflare \"Just a moment…\" wall), never for search -- it's slow and can stall.",
