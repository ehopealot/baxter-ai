// Tests for add-skill's pure, security-relevant cores: the grants.ts SKILL_NAMES
...
// run before use, not here. Importing add-skill.ts is side-effect-free (its
...
import { addSkillToGrants, parseSkillSpec, copyTree } from "./add-skill.ts";

const GRANTS = join(dirname(fileURLToPath(import.meta.url)), "grants.ts");
...
test("addSkillToGrants works on the REAL grants.ts (name lands at the end of the base list)", () => {
