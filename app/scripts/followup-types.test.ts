...
function direct(): Task {
  return {
    ...
    created_at: "2026-08-27T18:00:00.000Z", deliver: null,
    follow_up: {
      version: 1, subject: "the store trip", plan_date: "2026-08-28",
...
test("feature classifier routes every feature-shaped or unknown delivery record to strict handling", () => {
  ...
  assert.equal(isFeatureShapedTask({ ...ownUndefined, deliver: { surface: "mail-thread", target: "resend:a@example.com:x" } } as unknown as Task), true);
...
test("strict validator refuses malformed metadata, scheduler shape, route agreement, and authority", () => {
  const cases: Task[] = [
    ...
    { ...direct(), follow_up: { ...direct().follow_up!, subject_key: "STORE" } as never },
    { ...direct(), follow_up: { ...direct().follow_up!, plan_date: "2026-02-29" } },
    { ...direct(), follow_up: { ...direct().follow_up!, turn_token: "weak" } },
    { ...direct(), follow_up: { ...direct().follow_up!, origin: { surface: "mail-thread", id: "resend:not-an-email:abc" } } },
...
test("legacy duplicated routes and unknown variants fail closed", () => {
  const legacyDuplicatedRoute = { ...direct(), deliver: { surface: "mail-thread", target: "resend:member@example.com:abc" } } as unknown as Task;
  assert.equal(isFeatureShapedTask(legacyDuplicatedRoute), true);
  assert.throws(() => validateStoredFollowUp(legacyDuplicatedRoute));
