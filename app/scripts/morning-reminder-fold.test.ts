...
test("takes only direct one-shot reminders still pending before local noon on Monday or Friday", async () => {
  ...
  try {
    writeFileSync(join(dir, "schedule.json"), JSON.stringify([
      { id: "deadbeef", desc: "Send the Verizon phone back", at: "2026-08-24T16:00:00.000Z", next_run_at: "2026-08-24T16:00:00.000Z", deliver: { surface: "sms", target: "+15551234567" } },
    ...
    assert.deepEqual(await takeMorningRemindersForContact(contact, monday, "America/Los_Angeles"), [
...
test("leaves reminders alone outside Monday and Friday", async () => {
  ...
  try {
    ...
    assert.deepEqual(await takeMorningRemindersForContact(contact, new Date("2026-08-25T15:00:00.000Z"), "America/Los_Angeles"), []);
