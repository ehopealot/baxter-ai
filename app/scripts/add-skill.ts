...
// grants.ts's one shared SKILL_NAMES so every surface stages it.
//
...
// True iff `name` is already in grants.ts's SKILL_NAMES. Exported + reused by both
...
// Append `name` to grants.ts's `export const SKILL_NAMES = [ ... ]`. Pure +
...
function main(): void {
  ...
  if (hasSkill(grants, name)) throw new Error(`"${name}" is already a baked skill in grants.ts -- pick a different NAME`);
  ...
  // Wire grants.ts (append to the one shared list), then run the grants tests so a
  // bad edit fails HERE, not at the next daemon start.
  writeFileSync(GRANTS_PATH, addSkillToGrants(grants, name));
  console.error(`Appended "${name}" to grants.ts SKILL_NAMES.`);
  execFileSync("node", ["--test", join(APP_DIR, "scripts", "grants.test.ts")], { cwd: APP_DIR, stdio: "inherit" });
