...
import { loadSendState, MAX_SENDS_PER_DAY } from "./send-state.ts";
import { MEMORY_PATH, MEMORY_DIR, CREDENTIALS_PATH, LEARNED_SKILLS_DIR, AGENTMAIL_KEY_PATH } from "./paths.ts";
import { normalizeTranscriptText, neutralizeStructuralMarkers } from "./transcript.ts";
import { log, logErr, sh, ensureSkills, ensurePlaywrightConfig, runAgent, formatResetTime, fillTemplate, harnessLabel, skillsPreamble } from "./runtime.ts";
...
// grants.ts (one source of truth across poll/discord/heartbeat -- see the module
...
function renderPrompt(thread: ThreadOutput): string {
  ...
  // thread.from/thread.subject are the same raw JSON fields poll.ts itself uses
...
async function pollOnce(): Promise<void> {
  ...
  for (const threadId of idsByThread.keys()) {
    ...
    // mail.ts, not here) can push the count over the cap mid-cycle. Once
    ...
    const { outOfTokens, resetsAt } = await runAgent({
      ...
      // see the MEMORY_PATH comment in paths.ts) but cwd is MEMORY_DIR,
      ...
      // mail.ts is referenced by absolute path (in MAIL_TOOLS) since cwd is
      ...
      // can load a skill's full command reference on demand) -- see grants.ts.
...
async function main(): Promise<void> {
  ...
  // Persist the AgentMail API key (0600) so mail.ts reads it from a file rather
  // than the spawned run's env -- runAgent strips AGENTMAIL_API_KEY, so the run
  // reaches AgentMail only through the file-backed mail.ts CLI. Mirrors
  // discord-bot.ts's discord-token bootstrap. Guarded on the key being present.
