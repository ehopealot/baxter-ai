...
export function* walkFiles(start, cap, state) {
  ...
  while (stack.length) {
    ...
    // Sort descending and push so the stack pops them ascending (stable,
    // parent-before-children, alphabetical order in the output).
    entries.sort((a, b) => (a.name < b.name ? 1 : a.name > b.name ? -1 : 0));
    for (const d of entries) {
      if (d.isSymbolicLink()) continue; // never traverse or emit a symlink
      const abs = join(dir, d.name);
      if (d.isDirectory()) {
        if (!SKIP_DIRS.has(d.name)) stack.push(abs);
      } else if (d.isFile()) {
...
const USAGE = [
  ...
  "for regex or heavier parsing, use code-cli.",
].join("\n");
  ...
  try {
    ...
    if (cmd === "list") {
      ...
      if (results.length === 0) {
        ...
        for (const r of results) console.log(`${r.file}:${r.line}: ${r.text}`);
        if (truncated) console.log(`\n[capped at ${MAX_MATCHES} matches -- narrow the pattern or subpath]`);
      }
    } else {
      console.log(USAGE);
