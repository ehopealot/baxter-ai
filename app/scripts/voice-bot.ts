...
export function renderVoiceDispatchPrompt({ task, textChannelId, selfId }: { task: string; textChannelId: string; selfId: string }): string {
  return [
    ...
    `Discover ecosystem skills with \`skills-cli find <query>\` (see the skill-discovery skill): it searches the open agent-skills registry and returns metadata. You can only find + suggest to your operator -- there's no install verb. Never fetch or copy a third-party SKILL.md into your workspace.`,
  ].join("\n");
}
