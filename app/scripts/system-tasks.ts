// Compile-time registry of runtime-owned system tasks (2026-08-20 system
// scheduled tasks, T3): the closed key union, the definition/context/result
// contracts, the canonical `system:` id helper, and the strict-enabled
// predicate. The registry is the SOLE source of executable handler identity,
// description, and recurrence -- a persisted record only NAMES a key, and a key
// with no registered definition never executes code. The daily-calendar-digest
// definition is registered below at module load (SYSTEM_TASKS is initialized
// with it), while every consumer (T4/T5/T12) still takes the registry as an
// injectable parameter.
import type { Task } from "./schedule-store.ts";

export type SystemTaskKey = "daily-calendar-digest";
...
// The registry's first real member: the daily calendar digest (T11). The runtime
// import direction is system-tasks -> daily-calendar-digest; the handler imports
// only TYPES from this module, so there is no cycle.
import { dailyCalendarDigestDefinition } from "./daily-calendar-digest.ts";

// The daily calendar digest is registered at module load with the REAL defaults
// (its definition factory injects every seam); consumers still take the registry as
// an injectable parameter (default SYSTEM_TASKS) so tests inject their own.
export const SYSTEM_TASKS: readonly SystemTaskDefinition<SystemTaskKey>[] = [dailyCalendarDigestDefinition()];
