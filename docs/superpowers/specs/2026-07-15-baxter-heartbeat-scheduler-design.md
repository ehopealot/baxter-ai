...
- **Minimum recurrence interval — 1 hour** (`HEARTBEAT_MIN_INTERVAL_MINUTES`, default 60): `schedule-cli add` rejects a `--cron` whose next two consecutive occurrences are < 60 min apart (so `* * * * *` and the like are refused). At most ~24 fires/day per recurring task. **This applies to `--cron` only** — a one-shot `--at` has no minimum and can be scheduled for any time, even seconds out (it fires once, so there's no frequency to bound).
- **Max task count** (`HEARTBEAT_MAX_TASKS`, default 100): `add` rejects once the schedule is full.
- **Global daily fire cap** (`HEARTBEAT_MAX_FIRES_PER_DAY`, default 200): the driver stops firing (and logs `skipped`) once the day's fires are exhausted — a hard ceiling on total autonomous runs regardless of how many tasks exist.
...
- **Unit (`node:test`, the bulk — the store is pure/injectable):** `selectDue` (due vs future vs invisible), `claim` (sets window), `computeNextRun` (cron next in the task's `tz`; naive-`at`-in-`tz` vs offset-carrying-`at`; `HEARTBEAT_TZ` fallback), `onSuccess` (one-shot removed vs cron rescheduled), `onFailure` (attempts++, retry after window, give-up at max), **cancellation-wins** (`claim`/`onSuccess`/`onFailure` are no-ops when the task id is absent), **the limits** (a `--cron` with < 60-min spacing is rejected; a one-shot `--at` seconds out is accepted; `add` rejected at `HEARTBEAT_MAX_TASKS`; the driver stops firing + logs `skipped` at `HEARTBEAT_MAX_FIRES_PER_DAY`), `schedule-cli` arg parsing/validation (cron XOR at; delivery flags), and `withScheduleLock` mutual exclusion + atomic write (two concurrent writers don't lose an update or corrupt the file; a stale lock from a killed holder is recovered without double-acquisition).
