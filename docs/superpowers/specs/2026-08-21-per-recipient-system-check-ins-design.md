...
Each live handler invocation loads the allowlist and calls the existing `resolveRecipients` boundary before any model invocation. Its contact resolution, deduplication, and safe phone/email routing behavior remain unchanged; this design neither merges contacts nor redesigns recipient identity. The resulting deterministic contact order is the generation and delivery order for that invocation. A newly added contact after this snapshot waits for the next invocation. Provider sends still re-enter the existing fresh admission guards: a snapshotted recipient removed before `sendSms` performs its entry admission check is refused by SMS. Email admission is refused before `sendNew` dispatches only when the candidate is absent from the complete fresh `sendNew` authorization union: the fresh `loadAllowlist` result under its existing file/`ALLOWED_RECIPIENTS` seeding semantics, unioned with the current `OPERATOR_EMAIL` exception. Removing an operator address from `allowlist.json` alone does not revoke it while the same address remains authorized by `OPERATOR_EMAIL`; a revocation fixture must remove the address from every applicable source, including clearing or changing that exception. `resolveRecipients` and existing provider admission behavior remain unchanged. This design does not promise atomic revocation after either admission check while cap, moderation, or network work is in flight.
...
- fresh provider admission checks at `sendSms` entry and `sendNew` recipient resolution; email is refused only when the candidate is absent from the complete fresh `loadAllowlist`-plus-`OPERATOR_EMAIL` authorization union, without claiming atomic revocation after admission while cap, moderation, or network work is in flight; and
...
A fully processed invocation returns heartbeat success even when one or more contacts used a handled fallback. Specifically, handled quota denial, invalid output, model/provider failure, or out-of-tokens fallback must produce a final result with `ok: true` and `outOfTokens` and `deferredByCap` absent or false after all snapshotted contacts have completed their one delivery chain. This success contract is load-bearing: heartbeat clears the claim, advances cron, and does not retry the completed occurrence from contact zero. Only an unhandled handler failure or abort before the invocation is fully processed returns `ok: false` and retains the existing heartbeat failure/retry behavior.
...
- Durable-knowledge loading diagnostics are identity-free. They may report only the fixed source classes `memory`, `collections-root`, and `collection-entry`, plus aggregate counts grouped by a fixed failure category such as `missing`, `unreadable`, `symlink`, `invalid-type`, `mismatch`, `oversized`, or `raced`. They never interpolate a Collection slug, filename, filesystem path, source content, display name, or arbitrary exception message into a log or task detail. Multiple failures in the same source-class/category bucket produce one aggregate count rather than per-Collection diagnostics.
...
- Subjects, generated bodies, prompts, calendar fields, Collection slugs or paths, and durable-knowledge text never enter operational logs or task details.
...
16. Snapshot-versus-admission races: a recipient admitted only after generation begins receives neither a model call nor a send in that invocation. A recipient in the initial snapshot may have been generated for but is refused when removed immediately before `sendSms` performs its entry admission check. The email race removes the candidate from every member of the complete fresh `sendNew` authorization union before recipient resolution; when the fixture's address is also `OPERATOR_EMAIL`, it clears or changes that exception as well as removing the address from `allowlist.json`. It asserts refusal only when the candidate is absent from the whole union and does not claim that allowlist-file removal alone revokes an address still authorized by `OPERATOR_EMAIL`. The race fixtures exercise those concrete admission seams without asserting atomic revocation after either check.
17. One bounded attempt chain per contact index in a live invocation, no in-process revisit after handled failures, and heartbeat-level coverage for both completion and interruption semantics. In the completion fixture, one contact is delivered before a later model run reports out-of-tokens; fallback then completes that contact and all remaining contacts. Assert every contact is delivered exactly once, the handler returns `ok:true` with `outOfTokens` and `deferredByCap` absent/false, the claim clears, cron advances, attempts remain zero, and no retry starts again from contact zero. A separate crash/retry fixture documents that an unhandled interruption may begin a new invocation at contact zero because no durable completion state exists.
...
19. Durable-knowledge load diagnostics grouped only by fixed source class, fixed failure category, and aggregate count. A valid Collection slug equal to a household display name triggers a load diagnostic; assert that this slug/name, its path, and its content are absent from logs and task details while the applicable aggregate category/count may remain.
20. Zero resolved contacts causing zero reservations, model calls, and delivery-provider sends while allowing existing calendar refresh/fetch sequencing.
21. Memory and visible Collection fixtures retain existing sanitization, source ordering, framing, and byte ceilings.
22. Existing calendar selection, durable-knowledge bounds, system registry, schedule mirror, runtime dispatch, and provider admission behavior remaining intact.
...
- Partial quota/provider/model failures degrade to per-contact fallback without revisiting completed contact indices during the same live invocation. After all contacts are processed, handled quota denial, invalid output, model/provider failure, and out-of-tokens fallback return `ok:true` with `outOfTokens` and `deferredByCap` absent/false so heartbeat clears the claim, advances cron, and does not retry the completed occurrence; unhandled handler failure retains existing failure/retry semantics.
- A snapshotted recipient removed before the `sendSms` entry admission check is refused at that seam. Before `sendNew` dispatch, email is refused only if the candidate is absent from the complete fresh `loadAllowlist`-plus-`OPERATOR_EMAIL` authorization union; removing an address from `allowlist.json` alone is insufficient while the `OPERATOR_EMAIL` exception still authorizes it. No atomic revocation after either admission check is claimed, and `resolveRecipients` plus existing admission behavior remain unchanged.
- A crash or restart after an unhandled interruption may retry from contact zero under existing heartbeat semantics; no durable per-recipient completion guarantee is claimed.
...
- Logs, diagnostics, task details, and model metadata structurally exclude the `ResolvedContact` phone/email arrays; logs and task details also contain no names, content, calendar fields, durable knowledge, Collection slugs or paths, or free-form exception messages. Durable-knowledge loading reports only fixed source classes and aggregate counts by fixed failure category; other diagnostics remain limited to aggregate summaries and index/channel/fixed-category fields plus structurally accepted provider codes.
