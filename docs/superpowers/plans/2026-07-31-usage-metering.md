...
```
Return `usage` via a **conditional spread** — `return { ..., ...(usage ? { usage } : {}) }` — so a run with no result line (usage stays undefined) keeps the old shape and the `:118` fixture is unchanged (same pattern used in `detectRunnerOutcome`). **Update the two full-object `deepEqual` fixtures that DO carry a result line:** `:73` (success) gains `usage: { cost: null, inTok: 0, outTok: 0, src: "claude", model: "claude-sonnet-5" }` (that fixture's init event uses `claude-sonnet-5`; no cost fields on its result → cost null), and `:89` (error result, no init event) gains `usage: { cost: null, inTok: 0, outTok: 0, src: "claude", model: "" }`. Otherwise Step 4 goes red on pre-existing tests, not just the new one. Confirm live that `total_cost_usd` is on the terminal `result` line and `model` on the init event (the comment records the token-is-last-message assumption).
