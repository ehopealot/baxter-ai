...
| `make stop` | `COMPOSE_PROFILES="discord,heartbeat,mail,voice" docker compose down` the fleet — the fixed full profile set so every surface (incl. the opt-in poller AND voice) is torn down gracefully regardless of `BAXTER_SURFACES` (config volume + network left intact); also mops up any pre-compose containers of the same name |
| `make logs` | Follow logs from the fleet, incl. the poller + voice surface when up (`COMPOSE_PROFILES="discord,heartbeat,mail,voice" docker compose logs -f`; a bare `docker compose logs` is rejected by compose.yaml's `${PROJECT:?}` guards) |
...
**Discord is the default surface; the mail poller is opt-in.** Every surface is now a compose **profile** (`discord`, `heartbeat`, `mail`, `voice`; codapi is unprofiled so it always starts), and each `make` target sets `COMPOSE_PROFILES` itself — so a **bare `docker compose up` (no `COMPOSE_PROFILES`) starts only codapi**; always go through `make`. `make run` activates `BAXTER_SURFACES` (default `discord,heartbeat`), and `make run-mail` adds `mail`. Pass `BAXTER_SURFACES=<profiles>` to `make run` **as a make argument or exported in the environment** (the Makefile reads it via `?=`; it is NOT read from `app/.env` — that file is only the container `env_file`) to trim a fleet to fewer surfaces. It must NOT include `voice` — that needs a `VOICE=1` image build, so `make voice` owns it (the `check-surfaces` prereq rejects it fast). The poller is opt-in not for any maintenance reason (AgentMail is a single API key, no OAuth/token renewal), simply because Discord is the primary surface. Provision the inbox once with `make inbox`. See `README.md`.
