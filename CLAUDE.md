...
**Discord is the default surface; the mail poller is opt-in.** Every surface is now a compose **profile** (`discord`, `heartbeat`, `mail`, `voice`; codapi is unprofiled so it always starts), and each `make` target sets `COMPOSE_PROFILES` itself — so a **bare `docker compose up` (no `COMPOSE_PROFILES`) starts only codapi**; always go through `make`. `make run` activates `BAXTER_SURFACES` (default `discord,heartbeat`), and `make run-mail` adds `mail`. Set `BAXTER_SURFACES` in `app/.env` to trim a fleet to fewer surfaces (it must NOT include `voice` — that needs a `VOICE=1` image build, so `make voice` owns it). The poller is opt-in not for any maintenance reason (AgentMail is a single API key, no OAuth/token renewal), simply because Discord is the primary surface. Provision the inbox once with `make inbox`. See `README.md`.
